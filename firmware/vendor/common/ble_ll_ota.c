...
unsigned int otaLedFlag = 0;
unsigned long clk_OTA = 0;
void otaLedUI(){
	if(clock_time_ms() - clk_OTA >= 300){
		clk_OTA = clock_time_ms();
		otaLedFlag = !otaLedFlag;
		if(otaLedFlag){
...
{	
    if(blt_ota_finished_time){
        ...
        if(reboot_flag){
        	gpio_write(GPIO_PB6,1);
			RD_light_ota_with_sleep(3,1000*1000);
